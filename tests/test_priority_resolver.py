...
def test_priority_blocks_follow_expected_order(tmp_path: Path) -> None:
    ...
    blocks = resolver.build_priority_blocks(user_intent="Answer this question", memory_query="concise")

    assert len(blocks) >= 5
    assert blocks[0].startswith("# [P1] USER")
    assert blocks[1].startswith("# [P2] SOUL")
    assert blocks[2].startswith("# [P3] RELEVANT MEMORY")
    assert blocks[3].startswith("# [P4] AGENTS")
    assert blocks[4].startswith("# [P5] TOOLS")
